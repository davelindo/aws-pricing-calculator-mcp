...
import { listBlueprintCatalog, listServiceCatalog } from "./catalog.js";
import { fetchSavedEstimate, isOfficialCalculatorShareLink } from "./calculator-client.js";
import {
  attachPricingCommits,
  createCalculatorLinkResult,
  generateCalculatorLinkResult,
  validateFetchedEstimate,
} from "./calculator-link-runtime.js";
import { designArchitecture, priceArchitecture } from "./planner.js";
import {
  TOOL_CONTRACTS,
  normalizeToolError,
  normalizeToolOutput,
...
function successToolResponse(toolName, text, structuredContent, outputSchema = null) {
  const normalized = outputSchema
    ? outputSchema.parse(structuredContent)
    : normalizeToolOutput(toolName, structuredContent);
...
function errorToolResponse(toolName, error) {
  const message = error instanceof Error ? error.message : String(error);
  const hint = toolHint(toolName, message);
  const structuredContent = TOOL_CONTRACTS[toolName]
    ? normalizeToolError({
        contractVersion: "v1",
        tool: toolName,
...
export function createServer() {
  ...
  server.registerTool(
    ...
    withToolErrorHandling("generate_calculator_link", async ({ scenarioId, ...args }) => {
      const result = await generateCalculatorLinkResult({ scenarioId, ...args });
  ...
  server.registerTool(
    ...
    withToolErrorHandling("create_calculator_link", async ({ pricedScenario, pricingCommit }) => {
      const result = await createCalculatorLinkResult({ pricedScenario, pricingCommit });
